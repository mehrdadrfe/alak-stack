...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // Local diag (optional convenience)
  if (segs.length === 1 && segs[0] === '_diag') {
    const geo = (process.env.GEO_ORIGIN || '').replace(/\/+$/, '') || null
    res.status(200).json({ ok: true, GEO_ORIGIN: geo, node: process.version, pid: process.pid })
    return
  }
  ...
  try {
    let origin = process.env.GEO_ORIGIN || ''
    origin = origin.replace(/\/+$/, '')
    if (!origin) throw new Error('GEO_ORIGIN is not set')
